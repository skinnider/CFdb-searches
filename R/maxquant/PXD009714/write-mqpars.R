# Write mqpar files for the PXD009714 (Veyel et al., JBC 2018) dataset.
...
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = FALSE,
  ...
  extract(!grepl('20160714_dv_B04_2108', .))

# extract replicate and fractions
replicates = gsub("\\.raw", "", basename(files)) %>% 
  chartr('-', '_', .) %>% 
  strsplit('_') %>% 
  map_chr(4)
fractions = gsub("-.*$", "", basename(files)) %>% 
  chartr('-', '_', .) %>% 
  strsplit('_') %>% 
  map_chr(3)
