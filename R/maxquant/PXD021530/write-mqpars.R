# Write mqpar files for the PXD021530 (Luzarowski et al., Commun Biol 2021)
# dataset.
...
fraction_letts = basename(files) %>% 
  strsplit('_') %>% 
  map_chr(2) %>% 
  substr(1, 1)
fraction_idxs = basename(files) %>% 
  gsub("\\.raw$", "", .) %>% 
  strsplit('_') %>% 
  map_chr(2) %>% 
  substr(2, nchar(.)) %>% 
  ...
  str_pad(2, pad = '0') 
fractions = paste0(fraction_letts, fraction_idxs)
