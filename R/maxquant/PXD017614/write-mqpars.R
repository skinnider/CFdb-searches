# Write mqpar files for the PXD017614 (Szczepanowska et al., Nat Commun 2020)
# dataset.
...
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = FALSE,
                   recursive = TRUE) %>% 
  # filter one file MaxQuant cannot normalize
  extract(!grepl('20180427_A_CLPP_WT_01_NT_SG02', .))

# extract replicate and fractions
replicates = basename(files) %>% 
  gsub("^.*CLPP_|_NT.*$", "", .)
