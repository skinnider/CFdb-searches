...
# create grid
grid = tidyr::crossing(
  replicate = c('SEC1_Heterocyst', 'SEC1_Vegetative', 
                'SEC2_Heterocyst', 'SEC2_Vegetative', 
                'SEC3_Heterocyst', 'SEC3_Vegetative'),
...
grid0 = grid %>% 
  ...
  mutate(mq_dir = file.path(base_dir, 'MaxQuant/bin'),
         ...
         fasta_file = '~/git/CFdb-searches/data/fasta/filtered/UP000002483-Anabaena.PCC7120.fasta.gz'
  ) %>% 
