# Write mqpar files for the PXD023443 (Spaniol et al., J Exp Bot 2022) dataset.
...
# read files
files = list.files(args$raw_dir, pattern = "*\\.wiff$", full.names = FALSE,
                   recursive = TRUE)

# extract replicate and fractions
replicates = gsub("^.*Bbn", "", basename(files)) %>% 
  substr(1, 4) %>% 
  fct_recode('wtA' = 'Aida',
             'wtB' = 'Bida',
             'wtC' = 'Cida',
