# Write mqpar files for the PXD024625 (Nuebel et al., EMBO Rep 2021) dataset.
...
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = FALSE) %>% 
  # remove one file that MQ says is corrupt
  extract(!grepl('P18_011_Esther_c_mitos_2995_44', .))

# extract replicate and fractions
split = strsplit(basename(files), '_')
replicates = map_chr(split, 6)
fractions = map_chr(split, 7) %>% 
  gsub("\\.raw", "", .) %>% 
  as.integer() %>% 
