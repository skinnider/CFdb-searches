# Write mqpar files for the PXD022357 (Lee et al., Plant J 2021) dataset.
...
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = FALSE,
                   recursive = TRUE) %>% 
  # ignore some extras
  extract(!grepl('^O', basename(.))) %>% 
  # ignore one file that MaxQuant cannot normalize
  extract(!grepl('B1_F65_1_B', .))

# extract replicate and fractions
fractionation = ifelse(grepl('SEC_', basename(files)), 'SEC', 'IEX')
num = ifelse(grepl('^B1|^SEC_1', basename(files)), 1, 2)
replicates = paste0(
  fct_recode(fractionation, 'S' = 'SEC', 'I' = 'IEX'),
  'B', 
  num
)
split = basename(files) %>% 
  gsub("\\.raw", "", .) %>% 
  strsplit('_')
fractions = ifelse(fractionation == 'SEC', map_chr(split, 3), 
                   map_chr(split, 2)) %>% 
  gsub("F", "", .) %>% 
  as.integer() %>% 
