...
# cross-reference with Wan et al., 2015, and remove overlaps
...
design = data.frame(filename = unlist(expts1),
  ...
  mutate(fraction = n()) %>%
  ungroup() %>%
  # for Windows: append '_converted.mzXML'
  # mutate(filename = paste0(filename, '_converted.mzXML')) %>%
