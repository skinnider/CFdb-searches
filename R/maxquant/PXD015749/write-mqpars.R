# Write mqpar files for the PXD015749 (Alston et al., AJHG 2020) dataset.
...
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = FALSE,
                   recursive = TRUE) %>% 
  # ignore fraction control-61
  extract(!grepl("_61\\.raw", .))

# extract replicate and fractions
replicates = ifelse(grepl('Ctrl', files), 'Ctrl', 'Patient')
