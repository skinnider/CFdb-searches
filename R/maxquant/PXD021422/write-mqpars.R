# Write mqpar files for the PXD021422 (Floyd et al., bioRxiv 2020) 
# dataset.
...
split = strsplit(basename(files), '_')
condition = ifelse(grepl("Dep|ptase", basename(files)),
                   "Phosphatase", ifelse(grepl("Ctl|control", basename(files)),
                                         "Control", NA))
table(condition)
table(is.na(condition))
replicate = gsub("\\.raw", "", map_chr(split, 5))
fraction = ifelse(map_chr(split, 1) == 'Hu',
                  map_chr(split, 3),
                  map_chr(split, 4)) %>%
  gsub("a$", "", .) %>%
  paste0('F', .)
...
design = data.frame(file = files,
                    ...
                    experiment = fraction,
                    fraction = 1)
dplyr::count(design, replicate, condition)
# combine replicate and condition
design %<>% unite(replicate, replicate, condition)
