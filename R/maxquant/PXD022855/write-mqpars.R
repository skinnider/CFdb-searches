# Write mqpar files for the PXD022855 (Vidali et al., EMBO Mol Med 2021, mouse)
# dataset.
...
files = list.files(args$raw_dir, pattern = "*.raw$", full.names = TRUE) %>% 
  # remove one file MQ says is corrupt
  extract(!grepl("P18_046_SilviaVidali_Helmholtz_WT01_20_180605201535", .))

# parse replicates and fractions
replicates = ifelse(grepl("WT", basename(files)), "WT", "Hom")
