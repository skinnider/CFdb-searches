# Write mqpar files for the IPX0001620001 (Xu et al., Genomics Proteomics
# Bioinformatics 2021) dataset.
...
replicates = ifelse(grepl('sucrose', basename(files), ignore.case = TRUE),
                    'PCC6803_sucrose',
                    gsub(" .*$", "", basename(files)))
fraction = ifelse(grepl('sucrose', basename(files), ignore.case = TRUE),
                  strsplit(basename(files), '_') %>% 
                    map_chr(3, .default = NA_character_),
                  gsub("^.*\\(|\\).*$", "", basename(files))) %>% 
  as.integer() %>% 
  # make MaxQuant order fractions correctly
  str_pad(2, pad = '0') %>% 
  paste0('F', .)
