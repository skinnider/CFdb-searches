# Write mqpar files for the PXD033084 (Patron et al., EMBO J 2022) dataset.
...
# list files
files = list.files(args$raw_dir, full.names = TRUE, pattern = "*raw") %>% 
  # remove one file that MaxQuant cannot normalize
  extract(!grepl('190114_QE_Li_MariaP_Complexome_C68', .))

# parse replicates and fractions
split = basename(files) %>% 
  gsub("\\.raw", "", .) %>% 
  strsplit('_')
fractions = map_chr(split, 6)
replicates = substr(fractions, 1, 1)
fractions %<>% substr(2, nchar(.)) %>% 
