# Write mqpar files for the PXD028149 (Morgenstern et al., Cell Metab 2021)
# dataset.
...
summary = read.delim("data/mqpar/PXD028149/summary.txt.gz")

# limit to relevant fractions
summary0 = summary %>% 
  filter(grepl('BN_', Experiment))
replicates = strsplit(summary0$Experiment, '_') %>% 
  map_chr(2) %>% 
  paste0('BN_', .)
fractions = strsplit(summary0$Experiment, '_') %>% 
  map_chr(3) %>% 
  gsub("Slice", "", .) %>% 
  paste0('F', .)
files = file.path(args$raw_dir, paste0(summary0$Raw.file, '.raw'))
