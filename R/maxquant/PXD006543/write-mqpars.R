# Write mqpar files for the PXD006543 (Piazza et al., Cell 2018) dataset.
...
# read lists of files
veh_files = readLines(file.path(args$raw_dir, "Vehicle_files.txt")) %>%
  gsub("^.*\\\\", "", .)
atp_files = readLines(file.path(args$raw_dir, "ATP_files.txt")) %>%
  gsub("^.*\\\\", "", .)

# extract replicate and fractions
files = c(veh_files, atp_files)
replicate = c(rep('Vehicle', length(veh_files)), rep('ATP', length(atp_files)))
fraction = gsub("\\.raw", "", basename(files)) %>%
