# Write mqpar files for the PXD022856 (Vidali et al., EMBO Mol Med 2021, human)
# dataset.
...
# read summary file
summary = read.delim("data/mqpar/PXD022856/summary.txt.gz") %>% 
  filter(Experiment != "")

# parse replicates and fractions
files = file.path(args$raw_dir, paste0(summary$Raw.file, '.raw'))
replicates = substr(summary$Experiment, 1, nchar(summary$Experiment) - 2) %>% 
  fct_recode('Control' = 'C',
             'Patient1' = 'P1',
             'Patient2' = 'P2')
fractions = substr(summary$Experiment, nchar(summary$Experiment) - 1,
