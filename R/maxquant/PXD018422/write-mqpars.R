# Write mqpar files for the PXD018422 (Gerovac et al., RNA 2020) dataset.
...
# read metadata from summary file
summary = read.delim("data/mqpar/PXD018422/summary.txt") %>% 
  dplyr::select(Raw.file, Experiment) %>% 
  # drop pellet and 'S' fractions
  filter(!grepl('Pellet|S', Experiment),
         !grepl('Total', Raw.file)) %>% 
  # remove experiment that is not on PRIDE
  filter(!grepl('^848', Raw.file))
print(summary)

# extract replicate and fractions
replicates = ifelse(grepl('2019-05-02', summary$Raw.file),
                    'Gradient1', 'Gradient2')
fraction_map = summary$Experiment %>% 
  gsub("^.*_", "", .) %>% 
  as.integer() %>% 
  setNames(summary$Raw.file)
filenames = basename(files) %>% gsub("\\.raw", "", .)
fractions = fraction_map[filenames]
# drop files that aren't in fractions
files %<>% extract(!is.na(fractions))
fractions %<>% 
  extract(!is.na(fractions)) %>% 
