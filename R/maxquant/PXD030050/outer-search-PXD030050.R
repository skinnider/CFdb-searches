...
# create grid
replicates = list.files("data/mqpar/PXD030050", pattern = "xml") %>% 
  basename() %>% 
  gsub("^.*PXD030050-|\\.xml$", "", .)
grid = tidyr::crossing(
...
# also check for output files in git
output_files2 = grid %$%
  file.path("data/maxquant", experiment, replicate, 
            ifelse(phosphorylation, 'phosphorylation', 'default'),
...
grid0 = grid %>% 
  ...
  filter(!file.exists(output_file) & !file.exists(output_file2)) %>% 
  mutate(mq_dir = file.path(base_dir, 'MaxQuant/bin'),
         base_dir = file.path(base_dir, experiment, replicate),
