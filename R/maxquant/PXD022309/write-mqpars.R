# Write mqpar files for the PXD022309 (Skinnider et al., Cell 2021) dataset.
...
# list files
files1 = list.files(args$raw_dir, pattern = "*raw", full.names = TRUE)
files2 = list.files(args$raw_dir, pattern = "*dar", full.names = TRUE) %>% 
  # remove compression
  gsub("\\.1\\.dar", "", .)
files = c(files2, files1)

# parse replicates and fractions
split = basename(files) %>% gsub("\\.raw|\\.d.*$", "", .) %>% strsplit('_')
replicates = ifelse(grepl('raw$', files), 
                    map_chr(split, 5) %>% 
                      gsub("^s", "", .) %>% 
                      Hmisc::capitalize(),
                    map_chr(split, 3) %>% 
                      ...
                      substr(1, nchar(.) - 2)) %>% 
  paste0('2')
fractions = ifelse(grepl('raw$', files), 
                   map_chr(split, 7, .default = NA_character_) %>% 
                     gsub("Fraction", "", .) %>% 
                     gsub("\\.raw", "", .),
                   map_chr(split, 3) %>%
                     gsub("r$", "", .) %>% 
...
design = data.frame(file = files,
  ...
  filter(!grepl('Nsco_20190424_PCP_SILAC_kidney_rep2_Fraction32', file))
