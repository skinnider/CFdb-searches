...
# remove one file that causes issues for LFQ normalization
...
for (replicate in unique(replicates)) {
  ...
  # keep only the first 20 fractions, numerically
  # files0 = files[replicates == replicate]
  # files0 = head(files0, 20)
  
  # extract fractions
  experiments = gsub("\\.RAW$", "", basename(files0))
  # fraction_idxs = gsub("^.*_F", "", experiments) %>%
  #   as.numeric()
