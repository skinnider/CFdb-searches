# Write mqpar files for the PXD021500 (Stenton et al., JCI 2021) dataset.
...
# read summary files
summary = read.delim("data/mqpar/PXD021500/summary.txt.gz") %>% 
  drop_na(Experiment) %>% 
  filter(Raw.file != 'Total', Experiment != "") %>% 
  # remove one corrupt file
  filter(!grepl('P20_054_Revision_DNAJC30_patient_97816_20', Raw.file)) %>% 
  # remove one file that MQ cannot normalize
  filter(!grepl('P20_054_Revision_DNAJC30_patient_127242_29', Raw.file))

# get filepths, replicates
files = paste0(summary$Raw.file, '.raw') %>% 
  file.path(args$raw_dir, .)
replicates = summary$Experiment %>% gsub("_.*$", "", .)
fractions = summary$Experiment %>% 
  gsub("^.*_", "", .) %>% 
  paste0("F", .)
