# Write mqpar files for the PXD021451 (Lee et al., Svi Adv 2021, G. hirsutum)
# dataset.
...
files = list.files(args$raw_dir, pattern = "*.raw", full.names = TRUE) %>% 
  # remove one file that MaxQuant can't normalize
  extract(!grepl('B2_22_1_A', .))

# parse replicates and fractions
split = basename(files) %>% 
  gsub("\\.raw", "", .) %>% 
  strsplit('_')
replicates = map_chr(split, 1)
fractions = map_chr(split, 2) %>% paste0('F', .)
