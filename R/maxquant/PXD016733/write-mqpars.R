# Write mqpar files for the PXD016733 (Anand et al., LSA 2020, 60-fraction)
# dataset.
...
files = list.files(args$raw_dir, pattern = "*raw", full.names = TRUE) %>% 
  ...
  extract(!grepl('PXD016733/P18_016_MICOS_HAP1WT_02', .))

# parse replicates and fractions
replicates = rep(NA, length(files))
replicates[grepl('HAP1WT', files)] = 'WT'
replicates[grepl('Mic27', files)] = 'Mic27'
replicates[grepl('Mic26', files)] = 'Mic26'
replicates[grepl('Mic26_27', files)] = 'Mic26_27' ## overwrite prev line
fractions = basename(files) %>% 
  ## fix irregular delimiters
  gsub("26_27_", "2627_", .) %>% 
  gsub("\\.raw", "", .) %>% 
  strsplit('_') %>% 
  map_chr(5) %>% 
