# Write mqpar files for the PXD025571 (Li et al., PLoS ONE 2021) dataset.
...
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = FALSE,
                   recursive = TRUE) %>% 
  # filter to SEC
  extract(grepl('Bert_180208|Ernie_180410', .)) %>% 
  # drop two fractions that don't line up with PLoS ONE paper
  extract(!grepl('A00', .) &
            !grepl('A28_R1', .))

# extract replicate and fractions
replicates = gsub("_.*$", "", basename(files))
split = strsplit(basename(files) %>% gsub("\\.raw$", "", .), '_')
fractions = ifelse(replicates == 'Bert',
                   map_chr(split, 8, .default = NA_character_), 
                   map_chr(split, 6))
