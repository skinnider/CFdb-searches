# Write mqpar files for the PXD016469 (Daniels et al., J Proteome Res 2020) 
# dataset.
...
replicates = ifelse(grepl('_LPS', files), 'LPS', 'UT')
fractions = gsub("^.*_LPS|^.*_UT|\\.raw$", "", basename(files)) %>% 
  as.integer() %>% 
  str_pad(2, pad = '0') %>% 
  paste0('F', .)
