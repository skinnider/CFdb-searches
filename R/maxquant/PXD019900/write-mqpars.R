# Write mqpar files for the PXD019900 (Hor et al., NAR 2020) dataset.
...
# get fraction numbers from summary
summary = read.delim("data/supplements/Hor2020/summary.txt.gz")

# limit to relevant fractions
fractions = summary %>% 
  dplyr::select(Raw.file, Experiment) %>% 
  # drop pellet and input fractions
  filter(!grepl('Pellet|Input$', Experiment)) %>% 
  # drop empty rows
  filter(Experiment != "")
files = file.path(args$raw_dir, paste0(fractions$Raw.file, '.raw'))

# set replicate and fractions
replicate = 'No455'
fractions = fractions$Experiment %>% gsub("^.*,", "", .)
