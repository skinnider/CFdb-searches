# Write mqpar files for the PXD023017 (Ligas et al., Plant J 2019) dataset.
...
tissues = ifelse(grepl('^S', basename(files)), 'seedlings', 'leaves')
fraction_idxs = gsub("\\.raw$", "", basename(files)) %>% 
  substr(2, 3) %>% 
  as.integer()
fractions = (fraction_idxs %% 30) %>% replace(. == 0, 30) %>% 
  str_pad(2, pad = '0') %>% 
  paste0('F', .)
replicates = floor((fraction_idxs - 1) / 30) + 1
replicates %<>% paste0(tissues, .)
